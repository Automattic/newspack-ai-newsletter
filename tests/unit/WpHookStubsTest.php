 ...
 * plugin-file scope (newspack-ai-newsletter.php), so a stub that demands a
...
final class WpHookStubsTest extends TestCase {
	...
	public function test_add_action_accepts_lazy_class_string_callable(): void {
		...
		\add_action( $hook, [ '\\Newspack_AI_Newsletter\\Does_Not_Exist_Yet', 'register' ] );

		$this->assertContains(
			[ '\\Newspack_AI_Newsletter\\Does_Not_Exist_Yet', 'register' ],
	...
	public function test_add_filter_accepts_lazy_class_string_callable(): void {
		$hook = 'npainl_test_lazy_' . __FUNCTION__;
		\add_filter( $hook, [ '\\Newspack_AI_Newsletter\\Does_Not_Exist_Yet', 'filter' ] );

		$this->assertContains(
			[ '\\Newspack_AI_Newsletter\\Does_Not_Exist_Yet', 'filter' ],
			$GLOBALS['_wp_actions'][ $hook ]
		);
	}
}
