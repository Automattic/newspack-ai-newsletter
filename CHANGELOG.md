...
- **The GitHub source fetches releases + merged PRs only (10 per page).** `Github_Source_Node` no longer pulls the issues endpoint (the `issues()` method is kept, unwired, for now), trimming low-signal noise from the digest.
...
- **Digest no longer reloads a stale, ever-growing items list after a worker restart.** On FLUSH the `Digest_Builder` empties its accumulated items, but `scored:consumer`'s cursor didn't move, so its checkpoint (which only writes the offsetlog when the cursor advances) never co-committed the emptied snapshot — a restarted worker reloaded the full pre-FLUSH list and appended new items to it forever. The digest now takes the scored Partition (and a source `total`) as arguments (`make_node Digest_Builder digest scored:partition 3`) and, on FLUSH, sends the partition a throwaway `TM_INFO RESET` message that advances the consumer so its next checkpoint persists the emptied snapshot (it's ignored downstream — `handle_info` acts only on `DONE`). The `Digest_Builder` also **dedupes accumulated items by id** (cleared each FLUSH; a dirty restored snapshot is deduped too), so the same item can't appear twice in a digest.
